...
export interface CachedStickyMessage extends Entity {
	channelId: string;
...
export class StickyMessageRepository extends Repository {
	...
	async set(id: string, stickyMessageData: CachedStickyMessage) {
		await this.save(id, stickyMessageData);
	}

	async getAll(): Promise<CachedStickyMessage[]> {
		return (await this.search().return.all()) as CachedStickyMessage[];
	}
}
